...
class AddDecision extends Component {
    constructor(props) {
        ...
        const addAttribute = { error: {}, name: '', operator: '', value: '',name1: '',operator1: '',name2: ''};
    ...
    handleCancel() {
     ...
     addCustomFields() {
        this.setState({
            customCheck:  !this.state.customCheck
        });
     }
    ...
    mapNodeName(val) {
        const node = {};
        const { addAttribute: { name, operator, value, name1, operator1, name2, path }, attributes } = this.state;
        ...
        if (val === 'Add All' || val === 'Add Any') {
            ...
            //tbd
            if(this.state.customCheck){
                node['name'] = name;
                // let factValue = value.trim();
                let factValue = name1 + this.convertToSign(operator1) + name2;
    ...
    handleChildrenNode(value, customCheckBool) {
        ...
        if (value === 'Add Facts') {
            ...
            if (value === 'Add fact node') {
                const error = validateAttribute(this.state.addAttribute, attributes, customCheckBool);
    ...
    fieldPanel() {
        ...
        const customContent = this.state.customCheck ? <div className="add-field-panel"> 
            <div><SelectField options={attributeOptions} onChange={(e) => this.onChangeNewFact(e, 'name1')}
                    value={addAttribute.name1} error={addAttribute.error.name1} label="Custom Facts 1"/></div>
            <div><SelectField options={operatorOptions} onChange={(e) => this.onChangeNewFact(e, 'operator1')}
                    value={addAttribute.operator1} error={addAttribute.error.operator1} label="Custom Operator 1"/></div>
            <div><SelectField options={attributeOptions} onChange={(e) => this.onChangeNewFact(e, 'name2')}
                    value={addAttribute.name2} error={addAttribute.error.name2} label="Custom Facts 2"/></div>
        </div>: null;
        ...
        console.log(this.state.customCheck);
        ...
        return (<Panel>
            ...
            <div className="add-field-panel">
                ...
                <div><InputField onChange={(value) => this.onChangeNewFact(value, 'value')} value={addAttribute.value}
                        error={addAttribute.error.value} label="Value" placeholder={placeholder} required={false}/></div>
            </div>
            {/* <div> <Checkbox
                    label='Custom Facts and Values'
                    selected={this.state.customCheck}
                    onChange={this.handleCheckbox}/></div> */}

            <div className={`attributes-header ${background}`}>
                    <div className="attr-link" onClick={this.addCustomFields}>
                        <span className="plus-icon" /><span className="text">Add Custom Facts</span> 
                    </div>
            </div>
            {customContent}
            ...
            <div className="btn-group">
                    <Button label={'Add'} onConfirm={() => this.handleChildrenNode('Add fact node',this.state.customCheck) } classname="btn-toolbar" type="submit" />
